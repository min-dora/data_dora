...
data.thi <- data.month %>% mutate(THI = 1.8*평균기온-0.55*(1-습도/100)*(1.8*평균기온-26)+32)
...
data.thi %>% subset(THI_level=="매우높음")
...
data.thi %>% group_by(월) %>% summarise(THI_level_count = names(table(THI_level))[which(table(THI_level)==max(table(THI_level)))])
data.thi.group <- data.thi %>% group_by(월) %>% mutate(THI_level_count = names(table(THI_level))[which(table(THI_level)==max(table(THI_level)))])
# 변수 타입 확인
str(data.thi.group)
# THI_level_count 변수를 팩터로 변경 및 level설정
data.thi.group$THI_level_count <- data.thi.group$THI_level_count %>% factor(levels = c("낮음","보통","높음","매우높음"))

# ggplot
ggplot(data = data.thi.group)+geom_line(aes(x = 월, y = THI_level_count), group = 1)+geom_point(aes(x = 월, y = THI_level_count), size = 3)
